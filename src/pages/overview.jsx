...
export const Order = () => {
  ...
  const generateQRCode = async () => {
    try {
      if (upipayment === "upi") {
        //const response = await fetch(`https://test.ubsbill.com/apptest/sys/api/store-payments/generateQRCode?text=upi://pay?pa=${upi}%26am=${billtotal}%26cu=INR`);//ok
        const response = await fetch(`https://test.ubsbill.com/apptest/sys/api/store-payments/generateQRCode?text=upi://pay?pa=${upi}`);//ok
        if (response.ok) {
    ...
    const fetchFoodList = async () => {
      try {
        const response = await fetch(
          `https://test.ubsbill.com/apptest/sys/Food/foods/${currentuser.storeid}`
        );
  ...
  const fetchData = useCallback(() => {
    fetch(`https://test.ubsbill.com/apptest/sys/Food/foods/${currentuser.storeid}`)
  ...
  const handleSubmit = async (event, isKOTButton) => {
    ...
    try {
      ...
      console.log(payload);
      const response = await fetch("https://test.ubsbill.com/apptest/sys/Bill/create-bill", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const latestBillDataResponse = await fetch(`https://test.ubsbill.com/apptest/sys/Bill/getLastBill/${currentuser.storeid}`);
  ...
  payload.bill.paymentmode = bill.paymentmode;
      ...
      console.log(payload);
      const response = await fetch("https://test.ubsbill.com/apptest/sys/Bill/postbillOrder", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
payload.bill.paymentmode = bill.paymentmode;
  ...
  const fetchOrderList = async () => {
    try {
      const response = await fetch(
        `https://test.ubsbill.com/apptest/sys/Bill/billorder/${currentuser.storeid}`
      );
      if (!response.ok) {
  ...
  const fetchStoreLogo = useCallback(async () => {
    try {
      const response = await axios.get(`https://test.ubsbill.com/apptest/api/auth/store/${currentuser.storeid}/logo`, {
...
useEffect(() => {
    ...
    fetch(
     `https://test.ubsbill.com/apptest/sys/Inventory/minilevel/${currentuser.storeid}`
    )
     .then((response) => response.json())
     .then((data) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(`https://test.ubsbill.com/apptest/sys/Button/getbutton/${currentuser.storeid}`)
        .then((response) => response.json())
...
const handleMainReload = () => {
  ...
  useEffect(() => {
    const fetchFloorTables = async () => {
      try {
        const response = await fetch(
          `https://test.ubsbill.com/apptest/sys/floortable/getTablebystoreid/${currentuser.storeid}`
        );
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
      ...
      <div className="formbody overview animation">
        <form onSubmit={handleSubmit}>
          ...
          <div class="row22">
              ...
              >
                ...
                >
                  {foodList.map((food, index) => (
                    ...
                    >
                      <Card className={`my-2 cardss ${food.category === "veg" ? "veg-border" :
            ...
            <div class="row p-4" style={{ marginTop: "-5.2vh", marginLeft: "-38.5vw" }}>
              <div className="col-md-12" style={{ marginleft: "-50vw" }}>
                <div className="card  mt-3" style={{ marginLeft: "1vw", borderRadius: "5px", height: "72vh" }}>
                  <table
                    style={{
                      marginTop: "-1vh",
                      marginBottom: "2vh",
    ...
    <div className="form-check form-check-inline" style={{ marginLeft: "10px" }}>
      ...
      <label htmlFor="none" className="form-check-label font-weight-bold text-secondary">None</label>
                  ...
                  <div
                    ...
                    style={{height: "50vh",maxHeight: "50vh",overflowX: "hidden",marginleft: "auto",overflowY: "auto",}}>
              ...
              <div className="col" style={{ display: "flex", marginLeft: "1vh", marginTop: "-1.8vh" }}>
                <button
  ...
  <Modal.Footer style={{ justifyContent: "center" }}>
    ...
    <Button variant="primary" onClick={() => handlePrintKOT()}>
      ...
      <div ref={KOTContentRef} className="content-container">
        <h3 className="text-center kotf"> KOT </h3>
...
export const Order_list = () => {
 ...
 const fetchData = () => {
    const url = `https://test.ubsbill.com/apptest/sys/Bill/billorder/${currentuser.storeid}`;
...
const handleGeneratePDF = async () => {
    try {
     ...
     const formattedStartDate = startDate ? formatDate(startDate) : formatDate(defaultStartDate);
     const formattedEndDate = endDate ? formatDate(endDate) : formatDate(defaultEndDate);

     const response = await axios.post(`https://test.ubsbill.com/apptest/sys/order/generate-pdf-order/?store_id=${currentuser.storeid}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
     null,
     {
  ...
  const fetchAllOrders = () => {
    ...
    fetch(`https://test.ubsbill.com/apptest/sys/Bill/billorder/${currentuser.storeid}`)
        ...
        <div className="row rowleft6">
          ...
          <div style={{ width: "100vw", display: "flex", flexDirection: "row", justifyContent: "space-between", marginBottom: "2vh" }}>
            ...
            <div style={{ display: "flex", flexDirection: "row" }}>
             ...
             onClick={() => {
                window.location.href = "/overview/order";
             }}
            >
...
const handleFilter = (e, startDate, endDate) => {
  if(e.target.value === ''){
  ...
  setErrorShown(false); // Reset the error display flag when the filter is cleared
      ...
      const filteredByDate = searchApiData.filter((item) => {
       const isTodayBill = item.billdate === today;
       const isDateWithinRange = isDateInRange(item.billdate, startDate, endDate);
       ...
       return (
          (filteredOrders.length > 0 && isDateWithinRange) &&
          ((item.paymentmode || '').toLowerCase().includes(e.target.value.toLowerCase()) ||
           (item.id || '').toString().toLowerCase().includes(e.target.value.toLowerCase()) ||
           (item.total || '').toString().toLowerCase().includes(e.target.value.toLowerCase()) ||
           (item.discount || '').toString().toLowerCase().includes(e.target.value.toLowerCase()) ||
...
const handleGeneratePDF = async () => {
  try {
   ...
   const response = await axios.post(`https://test.ubsbill.com/apptest/sys/Bill/generate-pdf-bill/?store_id=${currentuser.storeid}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
   null,
   {
   responseType: 'blob', // Receive the response as a blob
   });
  ...
  const generateExcel = async () => {
    try {
        // Use the current date as default if startDate and endDate are not provided
        const defaultStartDate = new Date();
        const defaultEndDate = new Date();
        ...
        const response = await axios.post(
         `https://test.ubsbill.com/apptest/sys/Bill/generate-excel-bill/?store_id=${currentuser.storeid}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
         null,
         {
            responseType: "blob",
         }
...
export const Update_Order = () => {
  ...
  const handleSubmit = async (event, isKOTButton) => {
    ...
    try {
      const response = await fetch(
        "https://test.ubsbill.com/apptest/sys/Bill/updateBillorder/" + id,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        const latestBillDataResponse = await fetch(`https://test.ubsbill.com/apptest/sys/Bill/getBillByID/${id}`);
  ...
  const fetchOrderList = async () => {
    try {
      const response = await fetch(`https://test.ubsbill.com/apptest/sys/Bill/billorder/${currentuser.storeid}`);
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      const data = await response.json();
  ...
  const fetchTblNo = async (selectedId) => {
    try {
      const response = await fetch(`https://test.ubsbill.com/apptest/sys/Bill/getTblNoById/${selectedId}`);
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      const data = await response.json();
      ...
      <div className="formbody overview animation">
        <form onSubmit={(e) => handleSubmit(e)}>
          ...
          <div class="row22">
            <div class="col">
              {/*applied the search filter by neha */}
              <div style={{ display: "flex", justifyContent: "", marginTop: "1vh", marginLeft: "15vw" }}>
                ...
                <input
                  type="text"
                  placeholder=" &#128269; Search..."
                  style={{ border: "2px solid #03989e", fontSize: "2vh", width: "25vh", marginRight: "1vh" }}
                  value={searchInput}
                  onChange={handleSearchInputChange}
                />
              ...
              <div style={{ display: "flex", flexDirection: "row", flexWrap: "wrap" }}>
                ...
                <div style={{ marginBottom: "20px", marginTop: "-6.2vh", marginLeft: "8vh", marginRight: "3vw", height: "88vh", overflowY: "auto", scrollbarWidth: "0px", textAlign: "center" }} className="btn2">
                  <button
                    className={`btn ${selectedCategory === "All" ? "btn-success" : "btn-success"
                      } m-1`}
                    onClick={(e) => handleCategoryClick(e, "All")}
                ...
                <div className="row row77" style={{ height: "80vh", overflowY: "auto", scrollbarWidth: "0px", marginTop: "1vh", marginLeft: "-6vh", maxWidth: "54vw", width: "54vw" }}>
            ...
            <div class="row p-4" style={{ marginTop: "-2vh", marginLeft: "-37.5vw" }}>

              <div className="col-md-12" style={{ marginleft: "-50vw" }}>
                <div className="card  mt-3" style={{ marginLeft: "1vw", borderRadius: "5px", height: "73vh" }}>
                    ...
                    <tr>
                      ...
                      <td className=" col-md-6 text-center" style={{}}>
                        <input
                          ...
                          readOnly // Prevent user input
                          style={{ backgroundColor: "#fff", marginLeft: "0", width: "4.5vw" }}
                        />
                      </td>
                    </tr>
                    )}
